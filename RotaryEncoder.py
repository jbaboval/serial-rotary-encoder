...
class RotaryEncoder():
  ...
  def register_callbacks(self, turn=None, press=None):
    self.turn_cb = turn
    self.press_cb = press
    GPIO.add_event_detect(self.intr, GPIO.FALLING, callback=self._call_callbacks)
...
if __name__ == '__main__':
  re = RotaryEncoder([0x10, 0x11, 0x12], invert=True, factor=0.5)
  re.read_switches()
